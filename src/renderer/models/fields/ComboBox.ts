...
export const ComboBoxField = types
  ...
  .actions((self) => ({
    setCurrent(value: SnapshotIn<typeof ComboBoxValue>) {
      const key = currentValueKey(self);
      self.values.set(key, ComboBoxValue.create(value));
    },
    afterCreate() {
...
export const isEqualComboBox = (
  first: Instance<typeof ComboBoxValue>,
  second: Instance<typeof ComboBoxValue>
): boolean => {
